...
/*   Updated: 2025/10/29 13:43:31 by riel-fas         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_vertical_borders(t_data *data)
{
	...
	{
		line_len = ft_strlen(data->map[x]);
		if (line_len == 0)
		{
			x++;
			continue ;
		}
		if (data->map[x][0] != WALL && data->map[x][0] != ' ')
			return (FALSE);
		if (data->map[x][line_len - 1] != WALL &&
			data->map[x][line_len - 1] != ' ')
			return (FALSE);
		x++;
	}
	return (TRUE);
